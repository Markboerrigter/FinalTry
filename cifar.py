...
#########################################################################################################################
    ...
    def load(self):
        ...
        if self.from_pickle:
            try:
                with open(os.path.join(os.getcwd() +self.folder,self.pickle_data), 'rb') as fin:
                    self.cif = pickle.load(fin)
                    if self.verbose:
            ...
            filenames = [os.path.join(os.getcwd() + self.folder,f) for f in os.listdir(os.getcwd() + self.folder) if re.match(r'[0-9]+.*\.png', f)]
            ...
            fname_out = 'images-{}-{}-{}.p'.format(len(target), self.img_size, now)
            full_name = os.path.join(os.getcwd() + self.folder,fname_out)
            with open(full_name, 'wb') as fout:
                pickle.dump(self.cif, fout, -1)
