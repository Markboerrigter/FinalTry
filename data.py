...
########################################################################################################################
    ...
    def getRelativePath(self):
        ...
        - Hnd
        """
        mfiles = [os.path.join(os.getcwd() + self.folder_labels,mfile) for mfile in glob(os.path.join(os.getcwd() + self.folder_labels,'*.m'))]
        self.images = []
...
###################################################################################################################################################
    ...
    def getLabels(self):
        ...
        keys = range(1,63)
        values = list(map(str, list(range(10)))) + list(string.ascii_uppercase) + list(string.ascii_lowercase)
...
##################################################################################################################################################
    ...
    def load(self):
        ...
        if self.from_pickle:
            try:
                with open(os.path.join(os.getcwd() + self.folder_data,self.pickle_data), 'rb') as fin:
            ...
            for couple in complete:
                # print(couple)
                image = imread(os.path.join(os.getcwd() + self.folder_labels, couple[0] + '.png'), as_grey=True)
                sh = image.shape
            ...
            full_name = os.path.join(os.getcwd() + self.folder_labels,fname_out)
...
########################################################################################################################
    ...
    def merge_with_cifar(self):
        ...
        full_name = os.path.join(self.folder_data,fname_out)
        with open(full_name, 'wb') as fout:
            pickle.dump(cifar_plus_text, fout, -1)
